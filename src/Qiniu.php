...
class Qiniu extends Component {
	...
	/**
	 * 生成私有文件下载链接，并直接下载到本地路径
	 * @param string $fileNameList 私有文件链接
	 * @param bool|false $realDownload 是否直接下载
	 * @param string $downloadPath 下载文件保存路径
	 * @return string 下载文件链接，key为私有文件链接，value为临时下载链接
	 */
	public function download($fileName = '', $realDownload = false, $downloadPath = '') {
		if ($fileName === '') {
			return false;
		}
